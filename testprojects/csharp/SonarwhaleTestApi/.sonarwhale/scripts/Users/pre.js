...
function isTokenValid(token) {
    try {
        ...
        if (!claims.exp) return true; // no expiry claim — treat as valid
        return claims.exp > Math.floor(Date.now() / 1000) + 30; // 30s buffer
    } catch (e) {
        return false;
    }
}
...
if (!isTokenValid(token)) {
    ...
    var res = sw.http.post(
        ...
        JSON.stringify({ username: sw.env.get('username') || 'user', password: sw.env.get('password') || 'user123' }),
